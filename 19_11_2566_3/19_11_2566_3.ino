...
double RT_TEMP = 58.0; // Desired setpoint temperatures in Celsius
double INITIAL_DENATURE_TEMP = 68.0; // Desired setpoint temperatures in Celsius
double DENATURE_TEMP = 94.0; // Desired setpoint temperatures in Celsius
double ANNEALING_TEMP = 58.0; // Desired setpoint temperatures in Celsius
double EXTENTION_TEMP = 68.0; // Desired setpoint temperatures in Celsius
double FINAL_EXTENTION_TEMP = 68.0; // Desired setpoint temperatures in Celsius
//Time วินาที
int WARM_TEMP_TIME = 30;
int RT_TEMP_TIME = 30;
int INITIAL_DENATURE_TIME = 30;
...
void loop() {
  ...
  if (!isnan(max31855temp)) {  // ตรวจสอบว่าค่าไม่ใช่ NaN
    ...
    if (Input >= 10 && Input < 45) {
      ...
      //แก้  INITIAL_DENATURE_TEMP และ DENATURE_TEMP  = +7
      Input += 7;
    }
  }
...
void instrumentRun() {
  //PCR control
  //ที่ 95 องศาเซลเซียสค่าแก้ เท่ากับ 0
  //ที่ 50 องศาเซลเซียสค่าแก้ เท่ากับ 0
  //ที่ 72 องศาเซลเซียสค่าแก้ เท่ากับ 0
...
void runInstrument(){
//Protect >=100
if ((Input >= Setpoint) || (Input >= 100)) {
    analogWrite(peltierPin, 0); //ปรกติจะเท่ากับ 0
    analogWrite(fanPin, 80); //ปรกติ 80
  } else {
...
if ((Input >= Setpoint) || (Input >= 100)) {
    ...
    analogWrite(peltierPin, Output);
    analogWrite(fanPin, 200); //ทดสอบที่ความเร็วครึ่งนึง
  }
}
...
if ((Input >= Setpoint) || (Input >= 100)) {
    analogWrite(peltierPin, 0); //ปรกติจะเท่ากับ 0
    analogWrite(fanPin, 40); //ปรกติ 40
  } else {
    analogWrite(peltierPin, Output);
    analogWrite(fanPin, 120); //ทดสอบที่ความเร็วครึ่งนึง
  }
}
...
void stadyState() {
  ...
  if ((Input <= Setpoint + 0.5) && (Input >= Setpoint - 0.5)) {
    if (currentMillisStadyState - previousStadyState >= intervalStadyState) {
      ...
      if (measurementsCount >= 10) { //ให้ค่าเข่าใกล้ค่าคงที่มากที่สุดก่อนเพื่อจะเริ่มจับเวลา
        STATUS_CHECK = true;
        measurementsCount = 0; // รีเซ็ต measurementsCount เมื่อครบ 10 ครั้ง ใช้เวลาเพิ่ม 10 วินาที
      }
    }
  }
}
...
void stadyState_anneling() {
  ...
  if ((Input <= Setpoint + 0.5) && (Input >= Setpoint - 0.5)) {
    if (currentMillisStadyState - previousStadyState >= intervalStadyState) {
      ...
      if (measurementsCount >= 20) { //ให้ค่าเข่าใกล้ค่าคงที่มากที่สุดก่อนเพื่อจะเริ่มจับเวลา
        STATUS_CHECK = true;
        measurementsCount = 0; // รีเซ็ต measurementsCount เมื่อครบ 20 ครั้ง ใช้เวลาเพิ่ม 20 วินาที
      }
    }
  }
}
