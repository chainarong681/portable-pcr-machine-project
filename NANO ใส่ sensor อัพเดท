...
//LED สีน้ำเงินเป็นแสง excitation
int ledBluePin = 12;
...
void setup(void) {
  ...
  // Initialize the I2C port with custom SDA = 26 and SCL = 25 pins
  Wire.begin(26, 25);
  if (!as7341.begin()){
  ...
  //LED blue pin สำหรับ excitation
...
void timer_RUN() {
  if (millis() - last_time > period) {
    ...
    //ส่วนสร้างกราฟ
    if (currentIndex < timeRUN){ //เก็บค่าแค่ตาม setting ค่าเท่านั้น
      currentIndex++;
