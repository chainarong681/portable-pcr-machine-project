...
double WARM_TEMP = 94.0; // Desired setpoint temperatures in Celsius
...
double Kp = 40; // Proportional gain 40
  ...
  if (Input <= Setpoint + 0.5 && Input >= Setpoint - 0.5) {
    if (currentMillisStadyState - previousStadyState >= intervalStadyState) {
      ...
      if (measurementsCount >= 5) {
        STATUS_CHECK = true;
        measurementsCount = 0; // รีเซ็ต measurementsCount เมื่อครบ 5 ครั้ง
      }
    }
  }
}
