...
double RT_TEMP = 94.0; // Desired setpoint temperatures in Celsius
...
double Kp = 30; // Proportional gain 40
double Ki = 3; // Integral gain 3
...
void loop() {
  ...
  if (!isnan(max31855temp)) {  // ตรวจสอบว่าค่าไม่ใช่ NaN
    Input = max31855temp; //ส่งค่าที่ทำการวัด
  }
...
void runInstrument(){
//Protect >=99
if ((Input >= Setpoint + 0.2) || (Input >= 99)) {
    analogWrite(peltierPin, 0); //ปรกติจะเท่ากับ 0
    analogWrite(fanPin, 80); //ปรกติ 80
  } else {
    analogWrite(peltierPin, Output);
    analogWrite(fanPin, 255); //ทดสอบที่ความเร็วครึ่งนึง
  }
}
...
if ((Input >= Setpoint + 0.2) || (Input >= 99)) {
    ...
    analogWrite(peltierPin, Output);
    analogWrite(fanPin, 150); //ทดสอบที่ความเร็วครึ่งนึง
  }
}
...
void stadyState() {
  ...
  if (Input <= Setpoint + 0.5 && Input >= Setpoint - 0.5) {
    if (currentMillisStadyState - previousStadyState >= intervalStadyState) {
      ...
      if (measurementsCount >= 5) { //ให้ค่าเข่าใกล้ค่าคงที่มากที่สุดก่อนเพื่อจะเริ่มจับเวลา
        STATUS_CHECK = true;
        measurementsCount = 0; // รีเซ็ต measurementsCount เมื่อครบ 5 ครั้ง ใช้เวลาเพิ่ม 5 วินาที
      }
    }
  }
}
