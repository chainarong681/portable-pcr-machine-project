...
if ((Input >= Setpoint) || (Input >= 99)) {
    analogWrite(peltierPin, 0); //ปรกติจะเท่ากับ 0
    analogWrite(fanPin, 80); //ปรกติ 40
  } else {
...
//Protect >=99
double Setpoint_correction = Setpoint - 4.0; //แก้ค่า
if ((Input >= Setpoint_correction) || (Input >= 99)) {
    analogWrite(peltierPin, 0); //ปรกติจะเท่ากับ 0
    //ลดอุณหภูมิให้เร็วที่สุดก่อนถึงค่า Setpoint
    if (Input >= Setpoint_correction + 4){
...
void stadyState_anneling() {
  ...
  //แก้ค่าให้เริ่มการจับเวลาที่อุณหภูมิน้อยกว่า Seting -4 องศา เพื่อชดเชยอุณหภูมิในหลอดที่ลงตามไม่ทันที่ 25 ul
  double Setpoint_correction = Setpoint - 4.0; //แก้ค่า
  if (Input <= Setpoint_correction + 0.5 && Input >= Setpoint_correction - 0.5) {
    if (currentMillisStadyState - previousStadyState >= intervalStadyState) {
      measurementsCount++;
      previousStadyState = currentMillisStadyState;
