...
double ANNEALING_TEMP = 50.0; // Desired setpoint temperatures in Celsius
...
int EXTENTION_TEMP_TIME = 60;
int COOLING_TEMP_TIME = 300;
...
double Kp = 40; // Proportional gain 0
...
void loop() {
  ...
  if (!isnan(max31855temp)) {  // ตรวจสอบว่าค่าไม่ใช่ NaN
    Input = max31855temp + 1;  // กำหนดค่า tempRead เป็นค่าที่อ่านได้จาก MAX31855 (? = correction value)
  }
