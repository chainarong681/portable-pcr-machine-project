...
void loop() {
  ...
  if (!isnan(max31855temp)) {  // ตรวจสอบว่าค่าไม่ใช่ NaN
    ...
    if (Input >= 10 && Input < 45) {
      ...
      //แก้  INITIAL_DENATURE_TEMP และ DENATURE_TEMP  = -5 เดิม -6
      Input -= 5;
    }
  }
...
void stadyState_anneling() {
  ...
  if ((Input <= Setpoint + 0.5) && (Input >= Setpoint - 0.5)) {
    if (currentMillisStadyState - previousStadyState >= intervalStadyState) {
      ...
      if (measurementsCount >= 30) { //ให้ค่าเข่าใกล้ค่าคงที่มากที่สุดก่อนเพื่อจะเริ่มจับเวลา
        STATUS_CHECK = true;
        measurementsCount = 0; // รีเซ็ต measurementsCount เมื่อครบ 30 ครั้ง ใช้เวลาเพิ่ม 30 วินาที
      }
    }
  }
}
